...
Here is the exact pitch sequence to impress hackathon judges:

1. **The Hook (30 sec)**:
   > *"Judges, cloud outages cost thousands of dollars every minute. When `api-gateway` starts returning 502s at 3 AM, human on-call engineers spend 45 minutes digging through logs and wiki pages. Meet HealOps — an autonomous AI SRE built on AWS Strands Agents SDK that fixes outages in 4 seconds under strict Cedar safety guardrails."*

2. **The Outage (30 sec)**:
   - On the web dashboard at `http://localhost:8000`, point to the healthy green service topology.
   - Click the red **"Inject 502 Outage (api-gateway)"** button in the Chaos Lab.
   - Show the dashboard immediately turn red with an alert: `HTTP 502 Bad Gateway: Upstream connection pool exhausted`.

3. **The Agent in Action (45 sec)**:
   - Watch the **Strands Agent Stream**:
     - **Step 1**: Agent calls `get_system_telemetry` to check CPU and RAM.
     - **Step 2**: Agent calls `inspect_service_logs` and extracts the exact deadlock stack trace.
     - **Step 3**: Agent searches historical **Post-Mortem Runbooks**, matches incident `INC-104`, and finds the proven remediation: restart worker container.

4. **The Cedar Safety Intercept (45 sec)**:
   - Point out that the agent does NOT blindly execute destructive actions in production!
   - The screen highlights the **Cedar Policy Interception Card**:
     - *"Risk Level: CRITICAL. Remediation action 'restart_service' requires human SRE sign-off under Policy POL-003."*
   - Click **"✓ Approve & Auto-Remediate"**.

5. **The Healing & Learning (30 sec)**:
   - The agent restarts the service, validates health restoration, and returns the cluster to green.
   - Show the **SOC-2 Audit Trail** (cryptographically logged action) and the newly indexed **Post-Mortem Runbook** generated by the agent.
   - Conclude: *"HealOps transforms 45-minute outage fire drills into a 4-second autonomous, policy-guarded self-healing event."*
