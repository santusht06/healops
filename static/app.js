...
function initWebSocket() {
  ...
  ws.onopen = () => {
    console.log("[HealOps WS] Connected to agent event & log stream");
    wsIndicator.className = "connection-status connected";
    wsStatusText.textContent = "Live Socket";
    appendTerminalLine("info", `[SOCKET] Connected to HealOps central telemetry & live log streamer.`);
  };
...
function handleLiveEvent(msg) {
  ...
  switch (event_type) {
    ...
    default:
      console.log("[HealOps WS] Event:", event_type, data);
  }
}

// ==========================================
// Terminal Log Tail Streamer
// ==========================================
function appendTerminalLogEntry(entry) {
  if (!liveTerminalBody) return;

  const level = (entry.level || "INFO").toLowerCase();
  const time = entry.timestamp ? entry.timestamp.split("T")[1].replace("Z", "") : "";
  const text = `[${time}] [${entry.level}] ${entry.message}`;

  appendTerminalLine(level, text);
}

function appendTerminalLine(levelClass, text) {
  if (!liveTerminalBody) return;

  const line = document.createElement("div");
  line.className = `term-line ${levelClass}`;
  line.textContent = text;
  liveTerminalBody.appendChild(line);

  // Keep bounded
  while (liveTerminalBody.children.length > 120) {
    liveTerminalBody.removeChild(liveTerminalBody.firstChild);
  }

  liveTerminalBody.scrollTop = liveTerminalBody.scrollHeight;
}

function clearTerminal() {
  if (liveTerminalBody) {
    liveTerminalBody.innerHTML = `<div class="term-line info">[CLEARED] Live socket terminal cleared. Waiting for new output...</div>`;
  }
}
...
function renderServices(services) {
  ...
  services.forEach((s) => {
    const isDegraded = s.status === "degraded" || (s.health && (s.health.includes("502") || s.health.includes("DEADLOCK")));
    ...
    item.innerHTML = `
      <div class="service-meta-left">
        ...
        <div>
          ...
          <div class="service-meta-sub">${s.type || "microservice"} • :${s.port}</div>
        </div>
      </div>
      <div>
...
async function injectPaymentDeadlock() {
  try {
    ...
    console.error("Error triggering deadlock:", err);
  }
}
